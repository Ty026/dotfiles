...
function M.on_attach(client, buffer)
  ...
  self:map("gy", function() require("telescope.builtin").lsp_type_definitions({ reuse_win = true }) end,
	...
	local format = require("plugin.lsp.format").format
	...
	self:map("<leader>T", require("plugin.lsp.utils").toggle_diagnostics, { desc = "Toggle Inline Diagnostics" })
end
...
function M.rename()
	if pcall(require, "inc_rename") then
		return "ename " .. vim.fn.expand("<cword>")
	else
		vim.lsp.buf.rename()
	end
end
